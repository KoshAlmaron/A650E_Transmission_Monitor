...
ConfigComments = {'AfterChangeMinRPM' : 	'Если обороты после переключения будут ниже этого значения, то переключение блокируется.'
				...
				, 'RearGearInitMaxSpeed' :	'Задняя передача не включиться, если скорость превышает этот порог.'
				...
				, 'IdlePressureSLN' :	'ШИМ простоя (максимального давления подпора гидроаккумуляторов).'
				...
				, 'GlockMaxTPS' :		'Значение ДПДЗ, выше которого блокировка ГТ не включается.'
...
class _ConfigEditWindow:
	...
	def update_labels(self):	# Затычка для единообразия.
		pass

	def table_auto_update(self):	# Затычка для единообразия.
		pass
