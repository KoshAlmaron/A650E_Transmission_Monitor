...
ConfigComments = {'AfterChangeMinRPM' : 	'Если обороты после переключения будут ниже этого значения, то переключение блокируется.'
				...
				, 'SpeedCalcCoef' :		'Коэффициент для перевода скорости вращения выходного вала в скорость. Вычисляется по формуле (1 / [Передаточное число главной пары]) * [Длина окружности колеса] * 60 * 8.192. Округряем до ближайщего целого и записываем сюда. Для ВАЗ 2104, к примеру, коффициент получается - 227.'
