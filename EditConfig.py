...
ConfigComments = {'AfterChangeMinRPM' : 	'Если обороты после переключения будут ниже этого значения, то переключение блокируется.'
				...
				, 'AdaptationStepRatio' : 'При быстрой адаптации удваивается шаг смещения точки, что увеличивает скорость, но снижает точность работа алгоритма. Включать только при первоначальной настройке графиков.'
				...
				, 'G2AdaptTPSTempMin' :	'При температуре масла ниже этого порога адаптация по нагрузке переключается на адаптацию по температуре масла.'
				, 'G2AdaptTPSTempMax' :	'При температуре масла выше этого порога адаптация по нагрузке переключается на адаптацию по температуре масла.'
				...
				, 'G2AdaptTempMaxTPS' :	'Порог ДПДЗ (нагрузки), выше которого адаптация коррекции давления SLU по температуре отключается.'
				...
				, 'G2AdaptReactTempMin' :	'При температуре масла ниже этого порога адаптация по ускорению переключается на адаптацию по температуре масла.'
				, 'G2AdaptReactTempMax' :	'При температуре масла выше этого порога адаптация по ускорению переключается на адаптацию по температуре масла.'
				...
				, 'G3EnableAdaptTPS' :	'Включение алгоритма адаптации времени удержания SLU по нагрузке. Алгоритм работает в диапазоне температур, указанных ниже (>=, <=). Перед включением адаптации необходимо сначала примерно откатать график времени удержания SLU по нагрузке на прогретом масле и отключенной адаптации по температуре.'
				, 'G3AdaptTPSTempMin' :	'При температуре масла ниже этого порога адаптация по нагрузке переключается на адаптацию по температуре масла'
				, 'G3AdaptTPSTempMax' :	'При температуре масла выше этого порога адаптация по нагрузке переключается на адаптацию по температуре масла'
				, 'G3EnableAdaptTemp' :	'Включение алгоритма адаптации коррекции времени удержания SLU по температуре, алгоритм работает вне диапазоне температур для ускорения, указанных выше. Включать только после успешной настройки графика времени удержания SLU.'
				...
				, 'SpeedCalcCoef' :		'Коэффициент для перевода частоты вращения выходного вала в скорость. Вычисляется по формуле (1 / [Передаточное число главной пары]) * [Длина окружности колеса] * 60 * 8.192. Округляем до ближайшего целого и записываем сюда. Для ВАЗ 2104, к примеру, коэффициент получается - 227.'
				, 'MeterCalcCoef' :		'Коэффициент для расчета пробега по оборотам (x256). Вычисляется по формуле (1 / [Передаточное число главной пары]) * [Длина окружности колеса] * 256. Округляем до ближайшего целого и записываем сюда. Для ВАЗ 2104, к примеру, коэффициент получается - 118.'
				...
				, 'DefaultBaroPressure' :	'Стандартное атмосферное давление в кПа, относительно которого будет производиться коррекция.'
...
class _ConfigEditWindow:
	...
	def add_tooltip(self):	# Вставка подсказок.
		...
		ToolTip.ToolTip(self.WriteBtn, "Отправить настройки в ЭБУ. Настройки будут записаны в ОЗУ, для сохранения изменений необходимо сохhранить в EEPROM.")
	...
	def reset_config(self):	# Команда сброса таблиц в ЭБУ.
		if messagebox.askyesno('Сброс графиков', 'Перезаписать EEPROM ВСЕX графики текущего окна значениями из прошивки?', parent = self.root):
			time.sleep(0.5)
			self.Answer.update(1)
