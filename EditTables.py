...
class _TableEditWindow:
	def __init__(self, Uart):
		...
		self.root.title('Редакирование графиков')
	...
	def add_tooltip(self):	# Вставка подсказок.
		...
		ToolTip.ToolTip(self.AutoUpdateChk, "Автообновление графика. Период - 1с, доступно только для графиков адаптации.")
		
		ToolTip.ToolTip(self.ReadBtn, "Считать график из оперативной памяти ЭБУ")
		ToolTip.ToolTip(self.WriteBtn, "Отправить график в ЭБУ. График будет записана в ОЗУ, для сохранения изменений необходимо перенести график в EEPROM.")
		...
		ToolTip.ToolTip(self.ExportBtn, "Экспорт текущго графика в буфер обмена. Значения передаюся в буфер с разделителем-табуляцией для дальнейшей вставки в Excel.")
		...
		ToolTip.ToolTip(self.TableResetBtn, "Сброс всех графиков текущего окна. Значения заменяются на начальные из прошивки и производтся запись в EEPROM. Можно использовать в том числе для первоначальной записи графиков в EEPROM.")
		...
		ToolTip.ToolTip(self.BtnApplyAdapt, "Применение адаптации к текущему графику. После применения график адаптации обнуляется. Все изменения производятся в ОЗУ ЭБУ, для сохранения изменений необходимо произвести запись в EEPROM.")
	...
	def apply_backup_data(self):	# Применение данных из бэкапа к текущему графику.
		CurrTableName = TABLES.TablesData[self.CurrentTable]['Table']
...
class _Graph:
	def __init__(self, root, x, y, Uart):
		...
		self.GraphLines = []		# Текущий график
		self.GraphPoints = []
		...
		if self.GraphFocus:
			...
			if Button == 'Up':
				for i in range(Start, Stop + 1):
					Value = self.get_cell_value(Cells, i, 0)
